...
public:
	...
	T query(int v, int tl, int tr, int l, int r) {
	    if (l > r) 
	        return make_pair(-INF,0); //adaptar, caso outro tipo de consulta
	    if (l == tl && r == tr) {
	        return t[v];
	    }
	    int tm = (tl + tr) / 2;
...
int main() {
	SegTree st(6);
	vector<int> v{9,8,7,8,9,3};
	st.build(v);
	cout << st.query(0,5).first << " " << st.query(0,5).second << endl;
	cout << st.query(1,3).first << " " << st.query(1,3).second << endl;
	st.update(1,9);
	st.update(2,9);
	cout << st.query(1,3).first << " " << st.query(1,3).second << endl;
}
